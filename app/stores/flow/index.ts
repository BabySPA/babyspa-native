  ...
  persist(
    immer<FlowState>((set, get) => ({
      ...
      updateLingualImage: (name: string, url: string) => {
        return set((state) => {
          const idx =
            state.currentFlow.collect.healthInfo.lingualImage.findIndex(
              (item) => {
      ...
      updateOtherImage: (name: string, url: string) => {
        return set((state) => {
          const idx =
            state.currentFlow.collect.healthInfo.otherImages.findIndex(
              (item) => {
                if (typeof item === 'object') {
                  return item.name === name;
                }
          ...
          if (date) {
            ...
            if (dateRange[date]) {
              ...
              const applicationsCount =
                flow.analyze.solution.applications.reduce(
                  (sum: number, item: { count: number }) => {
                    return sum + item.count;
                  },
                  0,
